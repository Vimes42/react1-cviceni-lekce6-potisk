...
export const TShirtSetup = ({ onType, onColor, onPrint }) => {
  return (
    <form className="tshirt-setup">
      <label className="tshirt-setup__label">Typ:</label>
      <select className="tshirt-setup__input" onChange={(e) => {onType(e.target.value)}} >
      ...
      <select className="tshirt-setup__input" onChange={(e) => {onColor(e.target.value)}}>
      ...
      <input className="tshirt-setup__input" type="text" defaultValue="Ahoj světe" onChange={(e) => {onPrint(e.target.value)}} />
    </form>
  );
};
